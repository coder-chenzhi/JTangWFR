...
public class SimpleUndirectedGraphParser<NodeType, EdgeType> implements
	...
	public void serializeFragments(final OutputStream out,
		...
		for (final Fragment<NodeType, EdgeType> frag : frags) {
			output.write(serialize(frag.toGraph()).getBytes());		//序列化一个fragment
			output.write(("# => " + frag.frequency() + "[").getBytes());
			//找与之相关的图，例如[G532 ,G539]括号中的部分
...
//			HPGraph<NodeType, EdgeType> g = frag.toGraph().toHPGraph();
//			for(int i =0; i<g.getEdgeCount(); i++){
//				sb.append(g.getNodeLabel(g.getNodeA(i))+"->"+g.getNodeLabel(g.getNodeB(i))+"\n");
//			}
//			sb.append("}\n");
