 ...
 *            .equals(..))
 */
public class RecursiveStrategy4DFSCode<NodeType, EdgeType> implements
	...
	public Collection<Fragment<NodeType, EdgeType>> search(
		...
		int size = algo.returnInitials().size();
	...
	private void search(final SearchLatticeNode<NodeType, EdgeType> node,
		...
		if (node.store()
				&& node.toHPFragment().toHPGraph().getEdgeCount() == graphsize) {// 只挖掘待匹配图自身子图以及其DFSCode
			node.store(ret);
		} else {
			node.release();
		}
